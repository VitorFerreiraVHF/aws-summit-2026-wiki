# Operando a AWS com agentes especializados (patrocinado pelo TIVIT alma viva group) | AIM315-S

> Status: rascunho
> Dono:
> Ultima revisao: 2026-09-04
> Tags: `aws-summit-2026`, `sessao`, `aim315-s`

## Creditos

- Palestrante(s)/organizacao(oes): TIVIT
- Fonte: [Agenda oficial AWS Summit Sao Paulo 2026](https://aws.amazon.com/pt/events/summits/sao-paulo/agenda/)

## Metadados oficiais

| Campo | Valor |
| --- | --- |
| Codigo | AIM315-S |
| Horario | 12:45 BET |
| Local | Teatro Expo |
| Tipo | Palestra relâmpago |
| Nivel | 300 – Advanced |
| Topicos AWS | Inteligencia artificial |
| Areas de interesse | agentic-ai |
| Publico indicado |  |
| Industrias |  |
| Servicos AWS detectados |  |
| Formato/recursos | AWS Partners, Apresentacao, Lightning talk |

## Descricao oficial

Nesta palestra, você descobrirá como transformar a gestão de nuvem em um diferencial competitivo, conectando eficiência operacional e valor de negócio através de agentes autônomos de IA. Por meio de uma abordagem prática, aprofundaremos como criar um ecossistema multiagente especializado para acelerar a resolução de incidentes, garantir conformidade contínua e liberar equipes de TI do trabalho operacional para focar em inovação e crescimento de negócio.

## Topicos relacionados na wiki

[Generative AI e Machine Learning](../../03-topicos/generative-ai-machine-learning.md)

## Referencias oficiais relacionadas

- Nenhum servico AWS especifico foi detectado automaticamente nesta descricao.

## Evidencias locais

Adicione aqui fotos, videos, audios ou transcricoes relacionados a esta sessao.

| Evidencia | Tipo | Observacao |
| --- | --- | --- |
|  |  |  |

## Insights

| Insight | Evidencia | Impacto | Proxima acao |
| --- | --- | --- | --- |
|  |  |  |  |

## Agradecimento

Agradecimento aos palestrantes e organizacoes por compartilhar conhecimento com a comunidade AWS.
